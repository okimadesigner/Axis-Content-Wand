...
let currentSelectedFrameId = null;
// Enhanced state tracking for fixed text with expiration
let fixedTextRegistry = new Map();
// Session-based recently fixed tracking
let recentlyFixedInCurrentSession = new Map();
...
const createTextFingerprint = (text) => {
...
const loadNodeFonts = (figmaNode) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (fontName === figma.mixed) {
        ...
        for (let i = 0; i < figmaNode.characters.length; i++) {
            ...
            if (charFont === figma.mixed)
                continue;
            const exists = uniqueFonts.some(font => font.family === charFont.family && font.style === charFont.style);
            if (!exists)
                uniqueFonts.push(charFont);
        }
...
const createSelectionContext = (selection) => {
    if (!selection.length)
        return 'empty';
...
const isLikelyCompliantText = (text, nodeId) => {
...
const extractTextNodesFromFrame = (frame) => {
    const textNodes = [];
    const traverse = (node) => {
        var _a;
        if (!node.visible)
            return;
        if (node.type === 'TEXT') {
            const content = (_a = node.characters) === null || _a === void 0 ? void 0 : _a.trim();
...
const resetStateForNewSelection = (newSelectionContext) => {
        ...
        case 'analyze-selection':
            try {
                ...
                if (directTextNodes.length > 0) {
                    ...
                    // Look for frames
                    const frameNodes = selection.filter(node => node.visible && ['FRAME', 'COMPONENT', 'INSTANCE'].indexOf(node.type) !== -1);
                ...
                const needsAnalysis = textNodes.filter(node => !node.recentlyFixed && !node.likelyCompliant);
                const alreadyCompliant = textNodes.filter(node => node.recentlyFixed || node.likelyCompliant);
